...
work out [HH:MM]                   # 下班打卡（默认当前时间，可多次打卡以最后一次为准）
...
编辑 `batch_fix.sh`，在 `RECORDS` 数组中填写需要补录的记录，每行格式为 `YYYY-MM-DD HH:MM HH:MM`：

```bash
RECORDS=(
  "2026-02-03 08:30 19:00"
  "2026-02-04 08:45 18:30"
  "2026-02-05 09:00 20:00"
)
```
...
- **打卡状态**：`.work_start_time`（项目目录下，已加入 `.gitignore`）
- **加班记录**：`~/Documents/Obsidian/CDX/Overtime/YYYY-MM.md`（按月存储）
